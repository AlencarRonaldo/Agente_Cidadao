...
export const processError = (error) => {
  // Log seguro do erro
  try {
    console.error('Erro capturado:', getErrorMessage(error));
  } catch (logError) {
    console.error('Erro capturado: [Erro não serializável]');
  }
...
export const useErrorHandler = (componentName = 'Component') => {
  const handleError = (error, fallbackMessage = null) => {
    ...
    // Log estruturado para debug (seguro)
    console.group(`🚨 Error Handler [${componentName}]`);
    try {
      console.error('Original Error:', getErrorMessage(error));
    } catch {
      console.error('Original Error: [Erro não serializável]');
    }
