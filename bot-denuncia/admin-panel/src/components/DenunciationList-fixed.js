...
function DenunciationList({ token }) {
  ...
  // FIXED: Enhanced debug logging for development
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('🔧 DenunciationList DEBUG:', {
        hasToken: !!token,
  ...
  const loadDenuncias = useCallback(async () => {
    ...
    try {
      ...
      console.log('🔄 Loading denuncias with filters:', filters);
      ...
      // FIXED: Ensure data.data exists and is an array
      ...
      console.log('✅ Denuncias loaded successfully:', denunciasData.length);
      
    } catch (err) {
      console.error('❌ Error loading denuncias:', err);
      const errorMessage = err.message || 'Erro ao carregar denúncias';
      handleError(err);
  ...
  }, [page, rowsPerPage, filters, token, handleError]);
  ...
  const openDialog = useCallback((type, denuncia = null) => {
    console.log('🔧 openDialog called:', { type, denunciaId: denuncia?.id });
    
    if (!type) {
      console.error('❌ openDialog: No dialog type provided');
      return;
    }
    
    if (!denuncia && type !== 'batch') {
      console.error('❌ openDialog: No denuncia provided for individual action');
      return;
    }
    ...
    try {
      ...
      setError('Erro ao abrir diálogo: ' + error.message);
    }
  }, []);
  ...
  const handleImageClick = useCallback((imageUrl, event) => {
    console.log('🖼️ handleImageClick called:', { imageUrl, hasEvent: !!event });
  ...
  const ActionButtons = memo(({ denuncia, onOpenDialog }) => {
    ...
    if (!denuncia || !onOpenDialog) {
      console.warn('⚠️ ActionButtons: Missing props', { 
        denuncia: !!denuncia, 
